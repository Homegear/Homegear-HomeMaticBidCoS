...
class EasyCam : public BaseLib::Systems::DeviceFamily
{
public:
	EasyCam(BaseLib::Obj* bl, BaseLib::Systems::DeviceFamily::IFamilyEventSink* eventHandler);
	virtual ~EasyCam();
	virtual bool init();
	...
	virtual void load();
	virtual std::shared_ptr<EasyCamDevice> getDevice(uint32_t address);
	virtual std::shared_ptr<EasyCamDevice> getDevice(std::string serialNumber);
	virtual std::shared_ptr<BaseLib::Systems::Central> getCentral();
	virtual std::string handleCLICommand(std::string& command);
	virtual std::string getName() { return "EASYCam"; }
	virtual bool skipFamilyCLI() { return true; }
	virtual bool hasPhysicalInterface() { return true; }
	virtual PVariable getPairingMethods();
private:
	std::shared_ptr<EasyCamCentral> _central;

...
#include "../GD.h"
#include "EasyCamCentral.h"

namespace EasyCam {

EasyCamCentral::EasyCamCentral(IDeviceEventSink* eventHandler) : EasyCamDevice(eventHandler), BaseLib::Systems::Central(GD::bl, this)
{
	init();
}

EasyCamCentral::EasyCamCentral(uint32_t deviceID, std::string serialNumber, IDeviceEventSink* eventHandler) : EasyCamDevice(deviceID, serialNumber, eventHandler), Central(GD::bl, this)
{
	init();
}

EasyCamCentral::~EasyCamCentral()
{
	dispose();
}

void EasyCamCentral::init()
{
	try
	{
		EasyCamDevice::init();
...
void EasyCamCentral::deletePeer(uint64_t id)
{
	try
	{
		std::shared_ptr<EasyCamPeer> peer(getPeer(id));
		if(!peer) return;
		peer->deleting = true;
		peer->removeHooks();
		...
		_peersMutex.unlock();
		GD::out.printMessage("Removed EasyCam peer " + std::to_string(peer->getID()));
	}
	catch(const std::exception& ex)
    {
		_peersMutex.unlock();
...
std::string EasyCamCentral::handleCLICommand(std::string command)
{
	try
	{
		...
		if(command.compare(0, 12, "peers create") == 0 || command.compare(0, 2, "pc") == 0)
		{
			...
			{
				std::shared_ptr<EasyCamPeer> peer = createPeer(BaseLib::Systems::LogicalDeviceType(BaseLib::Systems::DeviceFamilies::EASYCam, deviceType), serialNumber, false);
		...
		else if(command.compare(0, 13, "peers setname") == 0 || command.compare(0, 2, "pn") == 0)
		{
			...
			{
				std::shared_ptr<EasyCamPeer> peer = getPeer(peerID);
				peer->setName(name);
...
std::shared_ptr<EasyCamPeer> EasyCamCentral::createPeer(BaseLib::Systems::LogicalDeviceType deviceType, std::string serialNumber, bool save)
{
	try
	{
		std::shared_ptr<EasyCamPeer> peer(new EasyCamPeer(_deviceID, true, this));
		peer->setDeviceType(deviceType);
		peer->setSerialNumber(serialNumber);
		peer->setRpcDevice(GD::rpcDevices.find(deviceType, 0x10, -1));
		if(!peer->getRpcDevice()) return std::shared_ptr<EasyCamPeer>();
    ...
    return std::shared_ptr<EasyCamPeer>();
}

bool EasyCamCentral::knowsDevice(std::string serialNumber)
{
...
bool EasyCamCentral::knowsDevice(uint64_t id)
{
	_peersMutex.lock();
	try
	{
...
PVariable EasyCamCentral::createDevice(int32_t clientID, int32_t deviceType, std::string serialNumber, int32_t address, int32_t firmwareVersion)
{
	try
	{
...
PVariable EasyCamCentral::deleteDevice(int32_t clientID, std::string serialNumber, int32_t flags)
{
	try
	{
		if(serialNumber.empty()) return Variable::createError(-2, "Unknown device.");
		std::shared_ptr<EasyCamPeer> peer = getPeer(serialNumber);
...
PVariable EasyCamCentral::deleteDevice(int32_t clientID, uint64_t peerID, int32_t flags)
{
	try
	{
...
PVariable EasyCamCentral::getDeviceInfo(int32_t clientID, uint64_t id, std::map<std::string, bool> fields)
{
	try
	{
		if(id > 0)
		...
		else
		{
			...
			std::vector<std::shared_ptr<EasyCamPeer>> peers;
			//Copy all peers first, because listDevices takes very long and we don't want to lock _peersMutex too long
			_peersMutex.lock();
			for(std::map<uint64_t, std::shared_ptr<BaseLib::Systems::Peer>>::iterator i = _peersByID.begin(); i != _peersByID.end(); ++i)
			{
				peers.push_back(std::dynamic_pointer_cast<EasyCamPeer>(i->second));
			}
			_peersMutex.unlock();

			for(std::vector<std::shared_ptr<EasyCamPeer>>::iterator i = peers.begin(); i != peers.end(); ++i)
			{
...
PVariable EasyCamCentral::putParamset(int32_t clientID, std::string serialNumber, int32_t channel, ParameterGroup::Type::Enum type, std::string remoteSerialNumber, int32_t remoteChannel, PVariable paramset)
{
	try
	{
		std::shared_ptr<EasyCamPeer> peer(getPeer(serialNumber));
		...
		{
			std::shared_ptr<EasyCamPeer> remotePeer(getPeer(remoteSerialNumber));
...
PVariable EasyCamCentral::putParamset(int32_t clientID, uint64_t peerID, int32_t channel, ParameterGroup::Type::Enum type, uint64_t remoteID, int32_t remoteChannel, PVariable paramset)
{
	try
	{
		std::shared_ptr<EasyCamPeer> peer(getPeer(peerID));

...
#ifndef EASYCAMCENTRAL_H_
#define EASYCAMCENTRAL_H_
...
class EasyCamCentral : public EasyCamDevice, public BaseLib::Systems::Central
{
public:
	EasyCamCentral(IDeviceEventSink* eventHandler);
	EasyCamCentral(uint32_t deviceType, std::string serialNumber, IDeviceEventSink* eventHandler);
	virtual ~EasyCamCentral();
	std::string handleCLICommand(std::string command);
	uint64_t getPeerIDFromSerial(std::string serialNumber) { std::shared_ptr<EasyCamPeer> peer = getPeer(serialNumber); if(peer) return peer->getID(); else return 0; }
...
protected:
	std::shared_ptr<EasyCamPeer> createPeer(BaseLib::Systems::LogicalDeviceType deviceType, std::string serialNumber, bool save = true);

    ...
    if (command.compare(0, 13, "channel count") == 0) {
      ...
      for (std::unordered_map<int32_t, std::vector<std::shared_ptr<BaseLib::Systems::BasicPeer>>>::iterator i = _peers.begin(); i != _peers.end(); ++i) {
        for (std::vector<std::shared_ptr<BaseLib::Systems::BasicPeer>>::iterator j = i->second.begin(); j != i->second.end(); ++j) {
          stringStream << "Channel: " << i->first << "\tAddress: 0x" << std::hex << (*j)->address << "\tRemote channel: " << std::dec << (*j)->channel << "\tSerial number: " << (*j)->serialNumber << "\tVirtual: "
                       << ((*j)->isVirtual ? "True" : "False") << std::endl << std::dec;
        }
      }
      _peersMutex.unlock();
      return stringStream.str();
  ...
  try {
    ...
    for (std::vector<std::shared_ptr<BaseLib::Systems::BasicPeer>>::iterator i = _peers[channel].begin(); i != _peers[channel].end(); ++i) {
      if ((*i)->address == address && (*i)->channel == remoteChannel) {
        _peers[channel].erase(i);
        peersGuard.unlock();
        if (linksCentral[channel].find(address) != linksCentral[channel].end() && linksCentral[channel][address].find(remoteChannel) != linksCentral[channel][address].end())
          linksCentral[channel][address].erase(linksCentral[channel][address].find(remoteChannel));
  ...
  try {
    ...
    for (BaseLib::Database::DataTable::iterator row = rows->begin(); row != rows->end(); ++row) {
      ...
      switch (row->second.at(2)->intValue) {
        case 1: _remoteChannel = row->second.at(3)->intValue;
          break;
        case 2: _localChannel = row->second.at(3)->intValue;
          break;
        case 4: _countFromSysinfo = row->second.at(3)->intValue;
          break;
        case 5: _messageCounter = row->second.at(3)->intValue;
          break;
        case 6: _pairingComplete = (bool)row->second.at(3)->intValue;
          break;
        case 7: _teamChannel = row->second.at(3)->intValue;
          break;
        case 8: _team.address = row->second.at(3)->intValue;
          break;
        case 9: _team.channel = row->second.at(3)->intValue;
          break;
        case 10: _team.serialNumber = row->second.at(4)->textValue;
          break;
        case 11: _team.data.insert(_team.data.begin(), row->second.at(5)->binaryValue->begin(), row->second.at(5)->binaryValue->end());
          break;
        case 12: unserializePeers(row->second.at(5)->binaryValue);
          break;
        case 13: unserializeNonCentralConfig(row->second.at(5)->binaryValue);
          break;
        case 14: unserializeVariablesToReset(row->second.at(5)->binaryValue);
          break;
        case 16:
          if (device) {
            pendingBidCoSQueues.reset(new PendingBidCoSQueues());
          ...
          break;
        case 17: _aesKeyIndex = row->second.at(3)->intValue;
          break;
        case 19: _physicalInterfaceID = row->second.at(4)->textValue;
          ...
          break;
        case 20: _valuePending = row->second.at(3)->intValue;
          break;
        case 21: _team.id = row->second.at(3)->intValue;
          break;
        case 22: _generalCounter = row->second.at(3)->intValue;
          break;
      }
    }
...
PVariable BidCoSPeer::getValueFromDevice(PParameter &parameter, int32_t channel, bool asynchronous) {
  try {
    ...
    if (!(getRXModes() & HomegearDevice::ReceiveModes::Enum::always) && !(getRXModes() & HomegearDevice::ReceiveModes::Enum::wakeOnRadio))
      return Variable::createError(-6,
                                   "Parameter can't be requested actively, because the device isn't reachable all the time.");
  ...
  try {
    ...
    if (rpcFunction->parameterGroupSelector && !rpcFunction->alternativeFunctions.empty()) {
      ...
      if (!parameter.rpcParameter) parameterGroup = rpcFunction->getParameterGroup(type);
        ...
        std::vector<uint8_t> parameterData = parameter.getBinaryData();
        int32_t index = parameter.rpcParameter->logical->type == BaseLib::DeviceDescription::ILogical::Type::Enum::tBoolean ? (int32_t)parameter.rpcParameter->convertFromPacket(parameterData,
                                                                                                                                                                                 parameter.mainRole(),
                                                                                                                                                                                 false)->booleanValue : parameter.rpcParameter->convertFromPacket(
            parameterData,
            parameter.mainRole(),
            false)->integerValue;
        if (index > 0) {
    ...
    for (std::vector<FrameValues>::iterator a = frameValues.begin(); a != frameValues.end(); ++a) {
      ...
      for (std::map<std::string, FrameValue>::iterator i = a->values.begin(); i != a->values.end(); ++i) {
        for (std::list<uint32_t>::const_iterator j = a->paramsetChannels.begin(); j != a->paramsetChannels.end(); ++j) {
          ...
          if (_bl->debugLevel >= 4)
            GD::out.printInfo(
                "Info: " + i->first + " on channel " + std::to_string(*j) + " of HomeMatic BidCoS peer " + std::to_string(_peerID) + " with serial number " + _serialNumber + " was set to 0x" + BaseLib::HelperFunctions::getHexString(i->second.value)
                    + ".");
...
PVariable BidCoSPeer::putParamset(BaseLib::PRpcClientInfo clientInfo, int32_t channel, ParameterGroup::Type::Enum type, uint64_t remoteID, int32_t remoteChannel, PVariable variables, bool checkAcls, bool onlyPushing) {
  try {
    ...
    if (type == ParameterGroup::Type::Enum::config) {
      ...
      for (auto &changedParameter : changedParameters) {
        //CONFIG_START
        payload.push_back(channel);
        ...
        payload.push_back(0);
        payload.push_back(0);
        payload.push_back(changedParameter.first);
        uint8_t controlByte = 0xA0;
        //Always send config start packet as burst packet => no ACK otherwise for some devices
        if (getRXModes() & HomegearDevice::ReceiveModes::Enum::wakeOnRadio) controlByte |= 0x10;
        ...
        for (std::map<int32_t, std::vector<uint8_t>>::iterator j = changedParameter.second.begin(); j != changedParameter.second.end(); ++j) {
...
PVariable BidCoSPeer::setValue(BaseLib::PRpcClientInfo clientInfo, uint32_t channel, std::string valueKey, PVariable value, bool wait) {
  try {
    ...
    values->push_back(value);
    if (_bl->debugLevel > 4)
      GD::out.printDebug("Debug: " + valueKey + " of peer " + std::to_string(_peerID) + " with serial number " + _serialNumber + ":" + std::to_string(channel) + " was set to " + BaseLib::HelperFunctions::getHexString(parameterData) + ".");
    ...
    for (BinaryPayloads::iterator i = frame->binaryPayloads.begin(); i != frame->binaryPayloads.end(); ++i) {
      ...
      if ((*i)->parameterId == "ON_TIME" && additionalParameter) {
        if ((rpcParameter->physical->groupId != "STATE" && rpcParameter->physical->groupId != "LEVEL") || (rpcParameter->physical->groupId == "STATE" && rpcParameter->logical->type != ILogical::Type::Enum::tBoolean)
            || (rpcParameter->physical->groupId == "LEVEL" && rpcParameter->logical->type != ILogical::Type::Enum::tFloat)) {
          GD::out.printInfo(
              "Info: Not setting auto reset after \"ON_TIME\" for " + rpcParameter->physical->groupId + ". Currently this is only supported for \"STATE\" of type \"boolean\" or \"LEVEL\" of type \"float\". Peer: " + std::to_string(_peerID)
                  + " Serial number: " + _serialNumber + " Frame: " + frame->id);
          continue;
        }
        std::vector<uint8_t> parameterData = additionalParameter->getBinaryData();
    ...
    if ((getRXModes() & HomegearDevice::ReceiveModes::Enum::always) || (getRXModes() & HomegearDevice::ReceiveModes::Enum::wakeOnRadio)) {
      ...
      if (!result) {
        if ((_deviceType == 0x19 || _deviceType == 0x26 || _deviceType == 0x27 || _deviceType == 0x28) && valueKey == "STATE" && value->booleanValue)
          pendingBidCoSQueues->remove(BidCoSQueueType::PEER,
                                      valueKey,
                                      channel); //Clear queue of KeyMatic and WinMatic when STATE was set to true;

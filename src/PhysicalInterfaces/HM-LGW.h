...
class HM_LGW : public IBidCoSInterface {
 ...
 protected:
  ...
  class AddPeerQueueEntry : public BaseLib::ITimedQueueEntry {
   public:
    AddPeerQueueEntry() {}
    AddPeerQueueEntry(PeerInfo &peerInfo, AddPeerQueueEntryType type, int64_t sendingTime) : ITimedQueueEntry(sendingTime) {
      this->peerInfo = peerInfo;
      this->type = type;
    }
    AddPeerQueueEntry(PeerInfo &peerInfo, int32_t channel, AddPeerQueueEntryType type, int64_t sendingTime) : ITimedQueueEntry(sendingTime) {
      this->peerInfo = peerInfo;
      this->channel = channel;
      this->type = type;
    }
    AddPeerQueueEntry(int32_t address, AddPeerQueueEntryType type, int64_t sendingTime) : ITimedQueueEntry(sendingTime) {
  ...
  class Request {
   public:
    ...
    Request(uint8_t responseControlByte, uint8_t responseType) {
      _responseControlByte = responseControlByte;

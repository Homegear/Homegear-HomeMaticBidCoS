...
IBidCoSInterface::IBidCoSInterface(std::shared_ptr<BaseLib::Systems::PhysicalInterfaceSettings> settings) : IPhysicalInterface(GD::bl, GD::family->getFamily(), settings), BaseLib::ITimedQueue(GD::bl, 1) {
  _bl = GD::bl;
  ...
  if (_oldRfKey.empty() && _currentRfKeyIndex > 1) {
    _out.printWarning(
        "Warning: The RF AES key index specified in homematicbidcos.conf for communication with your BidCoS devices is larger than \"1\" but \"OldRFKey\" is not specified. Please set your old RF key or - only if there is no old key - set key index to \"1\".");
  }
...
void IBidCoSInterface::startListening() {
  try {
    ...
    ITimedQueue::startQueue(0, 45, SCHED_FIFO);
  }
  catch (const std::exception &ex) {
    _out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
  }
...
void IBidCoSInterface::appendSignature(std::shared_ptr<BidCoSPacket> packet) {
  try {
    _aesHandshake->appendSignature(packet);
  }
  catch (const std::exception &ex) {
    _out.printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
  }

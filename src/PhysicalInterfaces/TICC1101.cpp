...
TICC1101::TICC1101(std::shared_ptr<BaseLib::Systems::PhysicalInterfaceSettings> settings) : IBidCoSInterface(settings)
{
	try
	{
		_out.init(GD::bl);
...
void TICC1101::disableUpdateMode()
{
	try
	{
		setConfig();
...
void TICC1101::setup(int32_t userID, int32_t groupID)
{
	try
	{
...
void TICC1101::sendPacket(std::shared_ptr<BaseLib::Systems::Packet> packet)
{
	try
	{
		if(!packet)
...
void TICC1101::readwrite(std::vector<uint8_t>& data)
{
	try
	{
		_sendMutex.lock();
...
bool TICC1101::checkStatus(uint8_t statusByte, Status::Enum status)
{
	try
	{
...
uint8_t TICC1101::readRegister(Registers::Enum registerAddress)
{
	try
	{
...
std::vector<uint8_t> TICC1101::readRegisters(Registers::Enum startAddress, uint8_t count)
{
	try
	{
...
uint8_t TICC1101::writeRegister(Registers::Enum registerAddress, uint8_t value, bool check)
{
	try
	{
...
void TICC1101::writeRegisters(Registers::Enum startAddress, std::vector<uint8_t>& values)
{
	try
	{
		if(_fileDescriptor->descriptor == -1) return;
...
uint8_t TICC1101::sendCommandStrobe(CommandStrobes::Enum commandStrobe)
{
	try
	{
		if(_fileDescriptor->descriptor == -1) return 0xFF;
...
void TICC1101::enableRX(bool flushRXFIFO)
{
	try
	{
		if(_fileDescriptor->descriptor == -1) return;
...
void TICC1101::initChip()
{
	try
	{
		...
		writeRegister(Registers::Enum::TEST2, 0x88, true); //Determined by SmartRF Studio
		writeRegister(Registers::Enum::TEST1, 0x31, true); //Determined by SmartRF Studio
		writeRegister(Registers::Enum::TEST0, 0x09, true); //Determined by SmartRF Studio
...
void TICC1101::startListening()
{
	try
	{
		...
		_listenThread = std::thread(&TICC1101::mainThread, this);
...
void TICC1101::mainThread()
{
    ...
    try
    {
		if(!_stopCallbackThread && (_fileDescriptor->descriptor == -1 || _gpioDescriptors[1]->descriptor == -1))
		{
			...
			std::thread thread(&TICC1101::startListening, this);
			thread.detach();
		}
